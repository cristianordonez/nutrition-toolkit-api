...
- Every supplied clinically significant weight loss or gain—including significance explicitly documented in a dated recent clinical fact, and including a change that lands exactly at a stated threshold (5%/1 month, 7.5%/3 months, 10%/6 months)—is interpreted using the full dated trend and current clinical picture, including whether the most recent weights show continued change or stabilization, and has an explicit `Weight change rationale:` based on documented evidence, a clearly qualified possible contributor, or the required undetermined-cause statement; no unsupported cause is asserted, and no qualifying significant weight change is left without this line.
