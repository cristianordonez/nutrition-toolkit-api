...
class Repository:
    def __init__(self, matches: list[RagSearchMatch]) -> None:
        ...
        self.ncp_args: tuple[str, int, str] | None = None
    ...
    def search_ncps(
        ...
        self.ncp_args = vector, top_k, person_identifier
        return self.matches
...
def test_embeds_documents_in_one_batch() -> None:
    ...
    embeddings = service.get_embeddings(["ncp", "knowledge"])

    assert embeddings == [[0.0], [1.0]]
    assert embedder.document_batches == [["ncp", "knowledge"]]
...
def test_embeds_documents_inside_running_event_loop() -> None:
    ...
    async def embed() -> list[list[float]]:
        return await service.get_embeddings_async(["ncp", "knowledge"])
...
def test_search_ncps_uses_query_embedding_scoped_to_one_person() -> None:
    document_id = 1
    ...
    matches = service.search_ncps("  weight loss  ", "R1", top_k=3)

    assert embedder.queries == ["weight loss"]
    assert repository.ncp_args == ("[0.1,0.2]", 3, "R1")
...
def test_search_ncps_inside_running_event_loop() -> None:
    ...
    matches = asyncio.run(
        service.search_ncps_async("weight loss", "R1", top_k=3),
    )
