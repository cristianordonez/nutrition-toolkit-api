...
from engine.models.extracted_fact_create import ExtractedFactCreate, LabPayload
...
PersonLab = tuple[int, str | None, str | None, date | None, LabPayload]
PersonLabKey = tuple[str | None, datetime | None, str, float | str]
...
class LabReportSplitter:
    """Split sequential PDF pages into logical person lab reports."""
    ...
    def split(
        ...
        *,
        ...
        for page_index in range(document.page_count):
            ...
            if metadata is None:
                if page_metadata["source_person_identifier"] is None and (
                    require_identity or page_metadata["collection_date"] is None
                ):
                    continue
    ...
    def _metadata(text: str) -> _LabReportMetadata:
        person = PccLabResultsExtractor._parse_person(text)  # noqa: SLF001
        if person is None:
            match = _BODY_PERSON_RE.search(text)
            person = (
        ...
        return {
            "source_person_identifier": person[0] if person else None,
            "source_person_name": person[1] if person else None,
    ...
    def _is_full_header(text: str) -> bool:
        return bool(
            PccLabResultsExtractor._parse_person(text)  # noqa: SLF001
    ...
    def _continues_report(
        ...
        if previous_page is not None and incoming_page != previous_page + 1:
            return False
        old_person = current["source_person_identifier"]
        new_person = incoming["source_person_identifier"]
        if (
            old_person is not None
            and new_person is not None
            and old_person.casefold() != new_person.casefold()
        ):
            return False
    ...
    def _merge_metadata(
        ...
        return _LabReportMetadata(
            source_person_identifier=current["source_person_identifier"]
            or incoming["source_person_identifier"],
            source_person_name=(
                current["source_person_name"] or incoming["source_person_name"]
            ),
    ...
    def _build_report(
        ...
        if metadata is None:
            msg = "Cannot build a lab report without person metadata"
            raise ValueError(msg)
        return LabReportDocument(
            source_person_identifier=metadata["source_person_identifier"],
            source_person_name=metadata["source_person_name"],
...
@register_extractor
class PccLabResultsExtractor(PersonExtractor):
    ...
    async def extract_for_demo(self) -> list[ExtractedFactCreate]:
        """Extract lab results without requiring source identity fields."""
    ...
    def _extract_pdf(self, *, require_identity: bool = True) -> list[PersonLab]:
        results: list[PersonLab] = []
        with pymupdf.open(self.path) as document:
            reports = LabReportSplitter.split(
                document,
                ...
                for report_page in report.pages:
                    ...
                    results.extend(
                        (
                            ...
                            report.source_person_identifier,
                            report.source_person_name,
    ...
    @staticmethod
    def _deduplicate(results: list[PersonLab]) -> list[PersonLab]:
        unique: list[PersonLab] = []
