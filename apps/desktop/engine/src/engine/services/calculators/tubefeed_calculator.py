...
class TubeFeedCalculator:
    """Provide stateless tube-feed methods, backed by a local formula catalog."""

    def __init__(
        self,
        formula_catalog: LocalFormulaCatalog,
    ) -> None:
        """Service handles tube feed calculations.

        :param formula_catalog: Local, non-PHI enteral-formula reference catalog
        """
    ...
    def find_formula(
        ...
        *,
        ...
        matches = list(
            self.formula_catalog.search_enteral_formulas(
                query,
    ...
    def calculate_nutrition(
        ...
        nutrient_amounts = {
            nutrient.number: nutrient.amount for nutrient in formula.nutrients
        }
        required_nutrients = {
            "energy": 1008,
            "protein": 1003,
    ...
    @staticmethod
    def _formula_display_name(formula: Formula, calories_per_ml: float) -> str:
        brand = formula.brand or formula.name
        return f"{brand} {calories_per_ml:.1f}".upper()

    @classmethod
    def _formula_choice(cls, formula: Formula) -> str:
    ...
    @staticmethod
    def _caloric_density(formula: Formula) -> float:
        ...
        calories = next(
            (
                ...
                if nutrient.number == 1008  # noqa: PLR2004
            ),
            None,
        )
        if calories is None:
