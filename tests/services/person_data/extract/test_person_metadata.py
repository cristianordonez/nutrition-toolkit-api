...
from ntk.models.extracted_fact_create import DietPayload, MiscOrderPayload
...
def test_pcc_person_headers_return_identifier_and_name() -> None:
    ...
    assert PccLabResultsExtractor._parse_person(text) == (  # noqa: SLF001
        "120046",
        "Zheng, Jing",
    )
    assert PccProgressNotesExtractor._parse_person(text) == (  # noqa: SLF001
        "120046",
        "Zheng, Jing",
    )


def test_deterministic_extractors_populate_person_demographics(
...
def test_order_rows_return_person_name_and_identifier() -> None:
...
def test_order_rows_do_not_treat_diagnosis_codes_as_persons() -> None:
    orders = PccOrderReportExtractor._parse_orders(  # noqa: SLF001
