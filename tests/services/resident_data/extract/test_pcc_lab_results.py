...
def _insert_header(  # noqa: PLR0913
    ...
    *,
    ...
    for index, value in enumerate(
        (
            "Lab Results Report",
            f"Resident: {resident} ({identifier})",
...
def test_name_alert_resident_header_is_not_replaced_by_lab_phone_number(
    tmp_path: pathlib.Path,
) -> None:
    ...
    assert reports[0].resident_identifier == "210486"
    assert reports[0].resident_name == "Patel, Maniben"
    assert {fact.facility_resident_identifier for fact in facts} == {"210486"}


def test_one_resident_multi_page_report_inherits_report_metadata(
    tmp_path: pathlib.Path,
) -> None:
    ...
    assert [fact.facility_resident_identifier for fact in facts] == ["RES1", "RES1"]
...
def test_same_resident_can_have_multiple_reports_with_missing_or_zero_orders(
    tmp_path: pathlib.Path,
) -> None:
...
def test_multiple_residents_transition_from_last_page_to_new_page_one(
    tmp_path: pathlib.Path,
) -> None:
    path = tmp_path / "all-residents-labs.pdf"
    with pymupdf.open() as document:
        first_page = document.new_page(width=700, height=500)
        _insert_header(
            first_page,
            resident="Agarwal, Kiran",
            identifier="EN140387",
