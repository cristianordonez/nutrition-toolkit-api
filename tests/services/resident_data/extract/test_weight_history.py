...
def test_all_residents_weight_state_continues_across_pages(
    tmp_path: pathlib.Path,
) -> None:
    path = tmp_path / "all-resident-weights.pdf"
    ...
    assert facts[1].resident_name == "Akhtar, Naseem"
    assert facts[-1].resident_name == "ANDERSON, BARBARA H"


def test_resident_header_at_page_end_applies_on_following_page(
    tmp_path: pathlib.Path,
) -> None:
    expected_height = 70
    path = tmp_path / "resident-header-continuation.pdf"
    with pymupdf.open() as document:
        first_page = document.new_page(width=900, height=700)
        _insert_lines(
            first_page,
    ...
    assert len(facts) == 1
    assert facts[0].facility_resident_identifier == "06785"
    assert facts[0].resident_name == "AIG, ABRAHAM"
    assert facts[0].height_in == expected_height
    assert facts[0].source_page == 2  # noqa: PLR2004


def test_weight_rows_deduplicate_by_resident_and_measurement_time(
    tmp_path: pathlib.Path,
) -> None:
    expected_weight = 120
    ...
    assert len(facts) == 1
    assert facts[0].facility_resident_identifier == "R1"
    assert isinstance(facts[0].payload, WeightPayload)
    assert facts[0].payload.weight_lb == expected_weight
