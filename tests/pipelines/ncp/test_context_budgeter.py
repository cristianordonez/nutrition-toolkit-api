...
def test_budgeter_maps_and_limits_agent_context() -> None:
    ...
    budgeted = ContextBudgeter().budget(
        detail,
        relevant_ncps=matches,
        additional_context=" focus " * 2_000,
    )

    context = budgeted.ncp_context
    assert len(context.relevant_ncps) == _BUDGETED_ASSESSMENT_COUNT
    assert context.relevant_ncps[0].chunk_text.endswith("[truncated]")
    assert context.additional_context is not None
    assert context.additional_context.endswith("[truncated]")
    assert budgeted.omitted_record_counts == {"relevant_ncps": 3}
