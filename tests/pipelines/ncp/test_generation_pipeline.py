...
from ntk.pipelines.ncp.create import pipeline as generation_pipeline
...
def test_generate_for_person_prepares_and_passes_budgeted_context(
    ...
    class Embeddings:
        @staticmethod
        def search_ncps(text: str, *, top_k: int) -> list[RagSearchMatch]:
...
def test_prepare_context_propagates_retrieval_failure() -> None:
    class Embeddings:
        @staticmethod
        def search_ncps(_text: str, *, top_k: int) -> list[RagSearchMatch]:
            assert top_k == 5  # noqa: PLR2004
