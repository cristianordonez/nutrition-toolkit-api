...
from ntk.pipelines.demo import ncp as demo
from ntk.pipelines.demo.ncp import DemoNutritionCareProcessPipeline, NoDemoFactsError
...
class NutritionCareProcessAgent:
    def __init__(self) -> None:
        self.context: BudgetedNCPContext | None = None
...
def test_demo_combines_facts_without_resolving_or_persisting_person(
    ...
    result = asyncio.run(
        _pipeline(embedding_service, ncp_agent).run_paths(
    ...
    assert result.ncp.id is None
    assert result.ncp.person_id is None
    assert result.ncp.content == "Generated from transient facts"
    assert ncp_agent.context is not None
...
def test_demo_uses_optional_identity_metadata_without_database_resolution(
    ...
    result = asyncio.run(
        _pipeline(EmbeddingService(), NutritionCareProcessAgent()).run_path(path),
    )
