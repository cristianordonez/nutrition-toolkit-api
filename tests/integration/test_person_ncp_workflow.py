...
from ntk.pipelines.ncp.create import pipeline as generation_service
from ntk.presentation.api.routers import documents, nutrition_care_processes
...
def test_ingestion_then_generation_uses_persisted_person_data(
    ...
    class Agent:
        ...
        async def run(
            ...
            assert ncp_context.additional_context == "annual nutrition review"
            cls.captured_payload = ncp_context.model_dump(mode="json")
    ...
    monkeypatch.setattr(generation_service, "NutritionCareProcessAgent", Agent)
    ...
    with Session(engine, expire_on_commit=False) as generation_session:
        ...
        assert generated[0].note_text == "Generated from persisted person data"
        assert (
            generation_session.exec(select(PersonNutritionClinicalNoteEmbedding)).all()
            == []
        )
