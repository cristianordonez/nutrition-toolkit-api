...
from ntk.agents import ncp_agent
from ntk.agents.ncp_agent import (
    NCPGenerationTimeoutError,
    NutritionCareProcessAgent,
)
from ntk.agents.tools import NCPToolDependencies
...
def _context() -> BudgetedNCPContext:
    return BudgetedNCPContext(
...
def test_ncp_agent_passes_prepared_context_and_tool_dependencies() -> None:
    ...
    assert payload["relevant_ncps"][0]["chunk_text"] == "Style example"
    assert payload["additional_context"] == "wound healing"
    assert isinstance(fake.deps, NCPToolDependencies)
...
def test_ncp_agent_registers_calculator_and_knowledge_tools() -> None:
    model = TestModel(call_tools=[], custom_output_text="done")
    dependencies = NCPToolDependencies(
        food_repo=typing.cast("typing.Any", _FOOD_REPO),
        embedding_service=typing.cast("typing.Any", _EMBEDDING_SERVICE),
    )

    with ncp_agent.ncp_agent.override(model=model):
        asyncio.run(
            ncp_agent.ncp_agent.run(
...
def test_ncp_agent_rejects_empty_note() -> None:
...
def test_ncp_agent_times_out_stalled_model(
    ...
    monkeypatch.setattr(
        ncp_agent,
        "_NCP_GENERATION_TIMEOUT_SECONDS",
        0.01,
    )

    with pytest.raises(NCPGenerationTimeoutError, match="timed out"):
        asyncio.run(
            NutritionCareProcessAgent(
...
def test_ncp_agent_requires_tool_dependencies() -> None:
...
def test_assessment_prompt_defines_budgeted_context_contract() -> None:
    prompt = (
        pathlib.Path(__file__).parents[2] / "src/ntk/agents/prompts/ncp_prompt.md"
    ).read_text(encoding="utf-8")

    assert "## Inputs and evidence" in prompt
    assert "`person`" in prompt
    assert "`relevant_ncps`" in prompt
