...
@register_extractor
class WoundReportExtractor(PersonExtractor):
    ...
    def _extract_report(
        ...
        """Extract each wound with the person identifier on its row."""
    ...
    def _parse_report(
        ...
        *,
        ...
        """Return all wound rows, optionally limited to one person."""
        header, rows = cls._wound_rows(text)
        if not header:
            return []

        selected_id = (
            source_person_identifier.strip() if source_person_identifier else None
        )
        observed_at = cls._parse_report_date(text)
        if observed_at is None:
            msg = "Wound report does not contain a source observation date"
