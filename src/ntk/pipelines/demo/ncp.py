"""Transient document-to-ncp demo orchestration."""
...
from ntk.models.demo_ncp import (
    DemoContextSummary,
...
class DemoNutritionCareProcessPipeline:
    """Extract documents and generate an ncp entirely in memory."""
    ...
    def __init__(
        ...
        *,
        ...
        ncp_agent: NutritionCareProcessAgent | None = None,
    ) -> None:
        ...
        self.ncp_agent = ncp_agent or NutritionCareProcessAgent(
            food_repo=food_repository,
            embedding_service=embedding_service,
        )

    async def run(self, upload: ReadableUpload) -> DemoNCPResponse:
    ...
    async def run_uploads(
        ...
        """Materialize uploads temporarily and generate an unsaved ncp."""
    ...
    async def run_path(self, path: pathlib.Path) -> DemoNCPResponse:
    ...
    async def run_paths(
        ...
        ncp_started_at = time.perf_counter()
        content = await self.ncp_agent.run(budgeted.ncp_context)
        logger.info(
            "Demo ncp generation completed: duration_seconds=%.3f",
            time.perf_counter() - ncp_started_at,
        )
        response = DemoNCPResponse(
            ...
            ncp=DemoGeneratedNCP.from_content(
                content,
        ...
        logger.info(
            "Demo ncp pipeline completed: documents=%s facts=%s duration_seconds=%.3f",
    ...
    async def _prepare_context(self, detail: PersonDetail) -> ContextBudgetResult:
        relevant_ncps = await self.embedding_service.search_ncps_async(
...
__all__ = ["DemoNutritionCareProcessPipeline", "NoDemoFactsError"]
